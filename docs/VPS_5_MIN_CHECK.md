...
```powershell
ssh root@YOUR_VPS_IP "systemctl is-active sro-bot"
```
...
```powershell
ssh root@YOUR_VPS_IP "systemctl restart sro-bot && systemctl status sro-bot --no-pager"
```

## 3) Last log lines

```powershell
ssh root@YOUR_VPS_IP "journalctl -u sro-bot -n 50 --no-pager"
```

## 4) Live logs (Ctrl+C to stop)

```powershell
ssh root@YOUR_VPS_IP "journalctl -u sro-bot -f"
```
...
ssh root@YOUR_VPS_IP "systemctl restart sro-bot"
```
